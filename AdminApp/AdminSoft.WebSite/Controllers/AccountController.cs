...
using PLNSecurity.ViewModels;
using PLNFramework.Security;
using PLNFramework.Security.Models;

namespace PLNSecurity.Controllers
{
    [Authorize]
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            //Realiza la validación de la activiación por correo electrónico
            var user = UserManager.FindByNameAsync(model.Email).Result;
            if (user != null)
            {
                if (!UserManager.IsEmailConfirmedAsync(user.Id).Result)
                {
                    ModelState.AddModelError("", "Confirme su dirección de correo electrónico haciendo clic en el enlace enviado a su correo.");
                    return View(model);
                }
            }
